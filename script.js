...
var transferAlerts = [];
/*
var transferAlert = setInterval(function(){
	var currentTime = new Date();
	currentTime = new Date(1970, 1, 1, currentTime.getHours(), currentTime.getMinutes(), 0);
	for(var ta=transferAlerts.length-1;ta>=0;ta--){
		var targetTime = new Date(1970, 1, 1, parseInt(transferAlerts[ta][1]), parseInt(transferAlerts[ta][2]), 0);
		if(currentTime.getTime() == targetTime.getTime()){
			alert("Check tank level!");
		}
	}
}, 60000);
*/
...
for (var l = 0; l < tankLevelInputs.length; l++) {
	tankInputs.push(tankLevelInputs[l]);
}
//tankInputs.push(...tankLevelInputs);
var tankReceivedInputs = document.getElementsByClassName("received");
for (var r = 0; r < tankReceivedInputs.length; r++) {
	tankInputs.push(tankReceivedInputs[r]);
}
...
function handleTime(element) {
	...
	if (mInt < 0) mInt = 0;
	if (mInt > 59) mInt = 59;
	var target = new Date(1970, 1, 1, hInt, mInt, 0);
	return target;
}
...
function calculateTransfer(transfer) {
	...
	if (storedPreviousFlowRate != null) {
		estimatesNonEditBoxes[0].innerHTML = storedPreviousFlowRate;
	}
	...
	/*
	for(var ta = 0; ta < transferAlerts.length; ta++){
		console.log("alert "+transferAlerts[ta][0]);
		if(transferAlerts[ta][0] == transferParentId){
			transferAlerts.splice[ta,1];
			break;
		}
	}
	*/
	var newAlert = [];
	newAlert.push(transferParentId);
	newAlert.push(estimatesTime.getHours());
	newAlert.push(estimatesTime.getMinutes());
	transferAlerts.push(newAlert);
	//console.log(transferAlerts);
	...
	estimatesNonEditBoxes[1].innerHTML = Math.round(estimatesDuration);
	estimatesNonEditBoxes[2].innerHTML = estimatesTimeString;
	estimatesNonEditBoxes[3].innerHTML = "?";
  ...
  */
	...
	var startLogTime = "";
	var commenceLogTime = "";
	var finishLogTime = "".concat(
		("0" + finalTime.getHours()).slice(-2),
		("0" + finalTime.getMinutes()).slice(-2),
		" - "
	);
	if (
		suctionTimeText == fillTimeText ||
		(isNaN(parseInt(suctionTimeElement.value)) == false &&
			isNaN(parseInt(fillTimeElement.value)))
	) {
		startLogTime = suctionTimeText;
	} else if (
		isNaN(parseInt(suctionTimeElement.value)) &&
		isNaN(parseInt(fillTimeElement.value)) == false
	) {
	...
	var finishLogText = "";
	if (
		selectedSuctionFromTankElement != null ||
		selectedSuctionToTankElement != null
	) {
	...
	if (isNaN(parseInt(finalTimeElement.value)) == false) {
		finishLogText = finishLogText.concat(
			finishLogTime,
			"Secured",
			" ",
			"filling",
...
function addTransfer() {
	var temp = document.getElementById("transferTemplate").innerHTML;
	//var clon = temp.cloneNode(true).firstElementChild;
	var transfersContainer = document.getElementById("transfersContainer");
	var index = transfersContainer.children.length;
	transfersContainer.insertAdjacentHTML("beforeend", temp);
	var clon = transfersContainer.children[index];
...
function deleteTransfer(deleteButton) {
	...
	transferParent.parentNode.removeChild(transferParent);
...
function loadTransfers() {
	...
	if (storedNumTransfers != null) {
		...
		for (var i = 0; i < storedNumTransfers; i++) {
			...
			var storedClassList = localStorage.getItem(
				transferParentId + "_collapsibleClassList"
			);
			if (storedClassList != null) {
				transfer.parentElement.children[0].children[1].classList = storedClassList;
			}
			if (
				transfer.parentElement.children[0].children[1].classList.contains("active")
			) {
			...
			var fillTimeElement = transfer.getElementsByClassName("fillTime")[0];
			var fillLevelElement = transfer.getElementsByClassName("fillLevel")[0];
			fillTimeElement.value = localStorage.getItem(transferParentId + "_fillTime");
