...
function calculateTransfer(transfer) {
  ...
  if (
    ...
    finishNonEditBoxes[1].innerHTML = flowRate.toFixed(1);
    finishNonEditBoxes[2].innerHTML = amountTransferred;
  ...
  var previousFlowRateElement = estimatesElement.getElementsByClassName(
    "previousFlowRate"
  )[0];
  localStorage.setItem(
    transferParentId + "_previousFlowRate",
    previousFlowRateElement.value
  );
  var storedPreviousFlowRate = localStorage.getItem("previousFlowRate");
  if (storedPreviousFlowRate != null) {
    previousFlowRateElement.placeholder = storedPreviousFlowRate;
  }
  previousFlowRate = parseFloat(previousFlowRateElement.value);
  if (isNaN(previousFlowRate))
    previousFlowRate = parseFloat(previousFlowRateElement.placeholder);
  ...
  var completeStartLog = "";
  var completeFillLog = "";

  var shiftedSuctionTimeText = suctionTimeText;
  if (
    isNaN(parseInt(suctionTimeElement.value)) &&
    isNaN(parseInt(fillTimeElement.value)) == false
  ){
    shiftedSuctionTimeText = fillTimeText;
  }
  if (
    selectedSuctionFromTankElement != null &&
    selectedSuctionToTankElement != null
  ) {
    completeStartLog = completeStartLog.concat(
      shiftedSuctionTimeText,
      " - Shifted ",
      suctionTankFromType,
      " suction from ",
      suctionTankFromName,
    ...
    if (!isNaN(parseInt(suctionLevelElement.value))) {
      completeStartLog = completeStartLog.concat(" (", suctionLevel, " gal).");
    } else {
      completeStartLog = completeStartLog.concat(".");
    }
  ...
  if (
    ...
    fillTimeText = suctionTimeText;
  }
  if(completeStartLog == ""){
    fillTimeLog = fillTimeLog.concat(fillTimeText, " - ");
  }else{
    fillTimeLog = fillTimeLog.concat("<br>", fillTimeText, " - ");
  }
  if (selectedFillTankElement != null) {
    completeStartLog = completeStartLog.concat(
      fillTimeLog,
    ...
    if (!isNaN(parseInt(fillLevelElement.value))) {
      completeStartLog = completeFillLog.concat(" (start: ", fillLevel, " gal)");
    }
    completeStartLog = completeFillLog.concat(".");
  }

  transfer.getElementsByClassName("logTextBox")[0].innerHTML = completeStartLog;
  transfer.getElementsByClassName(
    "logTextBox"
  )[1].innerHTML = "";
}
...
function addTransfer() {
  ...
  collapsible.addEventListener("click", function () {
    ...
    localStorage.setItem(
      this.parentElement.parentElement.id + " contentClassList",
      this.classList
    );
    if (this.classList.contains("active")) {
      content.style.display = "block";
...
function loadTransfers() {
  ...
  if (storedNumTransfers != null) {
    ...
    for (var i = 0; i < storedNumTransfers; i++) {
      ...
      previousFlowRateElement.value = localStorage.getItem(
        transferParentId + "_previousFlowRate"
      );
      var estimatesLevelPercentElement = estimatesElement.getElementsByClassName(
        "estimateLevelPercent"
