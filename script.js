...
function openFile(event){
	...
	var directoryPath = window.location.pathname.replace(/[^\\\/]*$/, '').substring(1);
	directoryPath = directoryPath.replace(/\//g,'\\');
	workbook = Excel.Workbooks.Open(directoryPath + filename);
	sheet = workbook.ActiveSheet;

	// setup the page to show relevant info from the excel sheet
	test();
}

function test() {
	sheet.Cells(1, 1).Value = "Hello";
	document.getElementById("testA").innerHTML = sheet.Cells(2,1);
	workbook.Save();
